    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="flex gap-8">
          ...
          <button
            onClick={() => setActiveTab("asignaciones_revisores")}
            className={`pb-4 text-sm font-medium border-b-2 transition-colors ${
              activeTab === "asignaciones_revisores"
                ? "border-blue-500 text-blue-600"
                : "border-transparent text-gray-500 hover:text-gray-700"
            }`}
          >
...
function UsuariosTab() {
  ...
  const [nuevoUsuarioCreado, setNuevoUsuarioCreado] = useState<{
    email: string;
    password: string;
  } | null>(null);
      ...
      {nuevoUsuarioCreado && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <p className="text-sm text-green-700 mb-2">
            Guarde estas credenciales y compartalas con el usuario:
          </p>
          <div className="bg-white rounded p-3 space-y-2">
            <div>
              <span className="text-sm font-medium text-gray-600">Email:</span>
              <code className="ml-2 text-sm bg-gray-100 px-2 py-1 rounded">
                {nuevoUsuarioCreado.email}
              </code>
            </div>
            <div>
...
function PuestosTab() {
  ...
  if (isLoading) {
    return (
      <div className="text-center py-8 text-gray-500">Cargando puestos...</div>
    );
  }
...
function AsignacionesTab() {
  ...
  const { data: asignaciones, isLoading: asignacionesLoading } =
    useAsignacionesPorPuesto(puestoId);
  ...
  const getMunicipioNombre = (municipioId: string) => {
    return (
      municipios?.find((m) => m.id === municipioId)?.nombre || "Desconocido"
    );
  };
      ...
      {!puestoId ? (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          <p className="text-gray-500">
            Seleccione un puesto de votación para ver sus mesas
          </p>
        </div>
      ) : isLoading ? (
        <div className="text-center py-8 text-gray-500">Cargando mesas...</div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          <p className="text-gray-500">
            Este puesto no tiene mesas registradas
          </p>
        </div>
      ) : (
        <div className="bg-white rounded-lg border border-gray-200">
                        ...
                        {estaEnEdicion ? (
                          <div className="flex items-center gap-2">
                            <select
                              ...
                              onChange={(e) =>
                                setTestigoSeleccionado(e.target.value)
                              }
                              className="text-sm border border-gray-300 rounded px-2 py-1"
                            >
                            ...
                            <button
                              ...
                              disabled={
                                !testigoSeleccionado || asignar.isPending
                              }
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                iniciarEdicion(mesa.id, testigoAsignado.id)
                              }
                            ...
                            <button
                              onClick={() =>
                                handleDesasignar(mesa.id, testigoAsignado.id)
                              }
...
function AsignacionesRevisoresTab() {
  const [municipioId, setMunicipioId] = useState<string>("");
  const [puestoEnEdicion, setPuestoEnEdicion] = useState<string | null>(null);
  const [revisorSeleccionado, setRevisorSeleccionado] = useState<string>("");

  const { data: municipios } = useMunicipios();
  const { data: revisores } = useRevisores();
  const { data: puestos, isLoading: puestosLoading } =
    usePuestosPorMunicipio(municipioId);
  const { data: asignaciones, isLoading: asignacionesLoading } =
    useAsignacionesRevisorPorMunicipio(municipioId);

  const asignar = useAsignarRevisorAPuesto();
  const desasignar = useDesasignarRevisorDePuesto();

  const getRevisorAsignado = (puestoId: string) => {
    const asignacion = asignaciones?.find((a) => a.puesto_id === puestoId);
    return asignacion?.revisor || null;
  };
  ...
  const handleDesasignar = async (puestoId: string, revisorId: string) => {
    await desasignar.mutateAsync({ revisorId, puestoId });
  };

  const iniciarEdicion = (puestoId: string, revisorActualId?: string) => {
    setPuestoEnEdicion(puestoId);
    setRevisorSeleccionado(revisorActualId || "");
  };
  ...
  const isLoading = puestosLoading || asignacionesLoading;

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900">
          Asignación de Revisores a Puestos
        </h2>
      </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {puestos?.map((puesto) => {
                  const revisorAsignado = getRevisorAsignado(puesto.id);
                        ...
                        {estaEnEdicion ? (
                          <div className="flex items-center gap-2">
                            <select
                              ...
                              onChange={(e) =>
                                setRevisorSeleccionado(e.target.value)
                              }
                              className="text-sm border border-gray-300 rounded px-2 py-1"
                            >
                            ...
                            <button
                              ...
                              disabled={
                                !revisorSeleccionado || asignar.isPending
                              }
                              className="px-2 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700 disabled:opacity-50"
                            >
                              {asignar.isPending ? "..." : "Guardar"}
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                iniciarEdicion(puesto.id, revisorAsignado.id)
                              }
                              className="text-blue-600 hover:text-blue-800 text-sm"
                            >
                              Cambiar
